...
class MainController extends Controller
{
    ...
    public function campaignsLog(Request $request, $source)
    {
        ...
        } else if ($source === "jakartahomelab") {
            $return = $this->logJakartaHomelab($source);
        } else {
    ...
    private function logWhiteningClinic($source)
    {
        $return = [];
    ...
    private function logDengue($source)
    {
        $return = [];
        $datas = DB::table('dengue_log')
            ->select('*')
            ->where('source', $source)
            ->orderBy('date', 'desc')
            ->get();
    ...
    private function logJakartaHomelab($source)
    {
        $return = [];
        $datas = DB::table('jakartahomelab_log')
            ->select('*')
            ->where('source', $source)
            ->orderBy('date', 'desc')
            ->get();
